...
public class VisuSqueeze implements MouseListener,MouseMotionListener,MouseWheelListener{
	...
	public VisuSqueeze(int w, int h, Data d, Color bg_c) {
		width = w;
		height = h;
		data = d;
		bg_color= bg_c;
...
//		
//		drawSections(pixels,data.compressed,off+width/4,width*height/3+off+width/4);
//		drawSections(pixels,data.compressed_w,width*height/3+off+width/4,width*2*height/3+off+width/4);
//		drawSections(pixels,data.compressed_diff,width*2*height/3+off+width/4,pixels.length);
//		
		drawCompressed(pixels,off,off+2*width*height/3);
		//for similarity sectioning
		drawData(pixels,data.sequences,off+width/2,width*height/3+off+width/2);
		drawData(pixels,data.sequences_w,width*height/3+off+width/2,off+2*width*height/3+width/2);
		drawData(pixels,data.sequences_diff,off+2*width*height/3+width/2,pixels.length+width/2);
		
//		
//		drawSections(pixels,data.scompressed,off+width/4+width/2,width*height/3+off+width/4+width/2);
//		drawSections(pixels,data.scompressed_w,width*height/3+off+width/4+width/2,width*2*height/3+off+width/4+width/2);
//		drawSections(pixels,data.scompressed_diff,width*2*height/3+off+width/4+width/2,pixels.length);
//		

		
		drawCompressedS(pixels,off+width/2,off+2*width*height/3+width/2);
	...
	public void drawSections(int[] pixels,Bundle seqs, int start, int end) {
		int top_off =start ;
		...
		for(int i = 0; i<sec.size();i++) {
			...
			drawLine(pixels,  (int) (width*sec_idx+top_off)-width/4,
					(int) (width*sec_idx+top_off)-width/4+getLength()-1, graph_color,1f);
...
//		drawBars(pixels, data.d_compressed_o, start+step, start+step/2+step);
	...
	public void drawBarsDenHomo(int[] pixels, Bundle seqs, int start, int end) {
	...
	public void drawDataBar(int[] pixels, Bundle seqs, int startpos,int dataRowIdx, int length) {
		// draw a bar with same data
		int col_hold =0;
		int diff_color = 0;
		for (int n = startpos; n<startpos+length*width&&n<pixels.length;n++) {
			int pos = ((n%width)-startpos%width);
			col_hold = data.getColor(seqs,dataRowIdx,pos);
			if(col_hold>=0) {
				diff_color =2*seqs.getDiff(dataRowIdx,pos);
				if(diff_color>255)diff_color =255;
				diff_color = new Color(diff_color/2,0,diff_color).getRGB();
				pixels[n] =mixColors(col_hold,diff_color,0.8f);
				}
		}
	}
	public void drawSqueezedBar(int[] pixels, Bundle seqs, int startpos,int sec_idx, int length) {
		// draw the into lenght SQUEEZED section of data
				int col_hold =0;
				// iterate over rows : start and end _idx
				// and then over individual data values
				for (int n =0; n<length;n++) {
					for(int i =0;i<data.getLength();i++) {
						col_hold = data.getSqueezeColor(seqs,sec_idx,n,i,length);
